...
if __name__=='__main__':
    ...
    plt.scatter(np.ones((1, len(R))) * R_x, R[:, 2], color='0.4')
    plt.scatter(np.ones((1, len(S))) * S_x, S[:, 2], color='0.6')
    ...
    for i in range(len(Q)):
        ...
        if i < len(Q)-1:
            plt.plot([Q_x, Q_x], [Q[i, 2], Q[i+1, 2]], color='blue', linewidth=4., label='Q', marker=None, linestyle='-')
    for i in range(len(R)):
        plt.text(R_x+0.2, R[i, 2], 'r' + str(i), color='0.4', fontsize=m_fontsize)
        if i < len(R)-1:
            plt.plot([R_x, R_x], [R[i, 2], R[i+1, 2]], color='0.4', linewidth=4., label='R', marker=None, linestyle='-')
    for i in range(len(S)):
        plt.text(S_x+0.2, S[i, 2], 's' + str(i), color='0.6', fontsize=m_fontsize)
        if i < len(S)-1:
            plt.plot([S_x, S_x], [S[i, 2], S[i+1, 2]], color='0.6', linewidth=4., label='S', marker=None, linestyle='-')
