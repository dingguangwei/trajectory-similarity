...
def get_data():
    ...
    value_20[:, 1] = [0.65,0.67,0.69,0.70,0.72,0.74,0.78,0.82]

    value_40 = np.zeros(shape=(len(x_list), 2))
    value_40[:, 0] = list(range(1, len(x_list) + 1))
    value_40[:, 1] = [0.82,0.84,0.88,0.9,0.92,0.95,0.96,0.97]

    value_60 = np.zeros(shape=(len(x_list), 2))
    value_60[:, 0] = list(range(1, len(x_list) + 1))
    value_60[:, 1] = [0.76,0.74,0.78,0.82,0.84,0.90,0.95,0.96]

    print(value_60)
    m_title='GeoLife'
    path = 'F:/temp/赵影毕设图片/exp09.jpg'
...
if __name__=='__main__':
    ...
    plt.ylabel('Average Server Load', fontsize=m_fontsize)
    # plt.title(m_title)
    plt.savefig(path)
