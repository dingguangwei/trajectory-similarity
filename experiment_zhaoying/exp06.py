...
def get_data():
    ...
    value_20[:, 1] = [0.16,0.20,0.32,0.42,0.60,0.72,0.88]

    value_40 = np.zeros(shape=(len(x_list), 2))
    value_40[:, 0] = list(range(1, len(x_list) + 1))
    value_40[:, 1] = [0.08,0.10,0.12,0.26,0.38,0.52,0.68]

    value_60 = np.zeros(shape=(len(x_list), 2))
    value_60[:, 0] = list(range(1, len(x_list) + 1))
    value_60[:, 1] = [0.14,0.20,0.26,0.38,0.52,0.64,0.76]

    print(value_60)
    m_title='GeoLife'
    path = 'F:/temp/赵影毕设图片/exp06.jpg'
