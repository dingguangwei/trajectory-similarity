...
def get_data():
    ...
    value_20[:, 1] = [0.42,0.38,0.32,0.25,0.23,0.18,0.10,0.08]

    value_40 = np.zeros(shape=(len(x_list), 2))
    value_40[:, 0] = list(range(1, len(x_list) + 1))
    value_40[:, 1] = [0.26,0.20,0.17,0.14,0.12,0.10,0.08,0.06]

    value_60 = np.zeros(shape=(len(x_list), 2))
    value_60[:, 0] = list(range(1, len(x_list) + 1))
    value_60[:, 1] = [0.38,0.32,0.27,0.23,0.18,0.10,0.08,0.06]

    print(value_60)
    m_title='GeoLife'
    path = 'F:/temp/赵影毕设图片/exp10.jpg'
...
if __name__=='__main__':
    ...
    plt.ylabel('Cache Hit Ratio', fontsize=m_fontsize)
    # plt.title(m_title)
    plt.savefig(path)
